...
with left,mid:
  ...
  def user_input_features():
    ...
    Weight= st.slider('น้ำหนัก (Kg)เท่าไหร่',0,150,79)
    st.write('น้ำหนัก', Weight, 'กิโลกรัม')
    st.write(' # --------------------------------------')
    Cycle = st.slider('ประจำเดือนมากี่วัน',0,31,7)
    st.write('รอบเดือนมา', Cycle, 'วัน')
    st.write(' # --------------------------------------')
    CycleLength= st.slider('ระยะห่างของรอบเดือน ห่างกันกี่วัน',0,60,16)
    st.write('ระยะห่างของรอบเดือน', CycleLength, 'วัน')
    st.write(' # --------------------------------------')
    st.write('### สอบถามอาการ ใช่(1)หรือไม่(0) ')
    hairGrowth = st.slider('ขนตามจุดต่างๆเพิ่มขึ้นหรือไม่ ',0,1,1)
